...
int main(int argc, const char * argv[])
{
    ...
    for (int i = 0; i<NUM_FILES; i++) {
        char fName[200];
        snprintf(fName, sizeof(char) * 200,"%s/ion.test.lab", rootDirectory);
    ...
    for (int i = 0; i<NUM_FILES; i++) {
        char fName[200];
        snprintf(fName, sizeof(char) * 200,"%s/ion.test.fea", rootDirectory);
        pFile = fopen(fName,"rb");
        int n;
        fread(&n,4,1,pFile);
        n = ntohl(n);
        for (int j = 0; j < n; j++)
        {
            ...
            features[(i * ROWS_PER_FILE) + j][FEATURE_COLS] = 1.0f;
        }
        fclose(pFile);
    }
    if (debug) {
    ...
    for (int i = 0; i<NUM_HELDOUT_FILES; i++) {
        ...
        for (int j = 0; j < n; j++)
        {
            ...
            heldFeatures[(i * ROWS_PER_FILE) + j][FEATURE_COLS] = 1.0f;
        }
        fclose(pFile);
    }
    if (debug) {
    ...
    for (int j = 0; j < NUM_FILES * ROWS_PER_FILE; j++)
    {
        ...
        for (int d = 0; d < (FEATURE_COLS + 1); d++)
        {
            regTerm += powf(beta[labels[j] - 1][d],2);
        }
        float temp1 = dotProductV[labels[j] - 1] - (maxDotProduct + logf(z)) - (REG_PARAM * regTerm);
        float temp2 = (maxDotProduct + logf(z)) - (REG_PARAM * regTerm);
        float temp3 = dotProductV[labels[j] - 1];
        logLikelihood += dotProductV[labels[j] - 1] - (maxDotProduct + logf(z)) - (REG_PARAM * regTerm);
    }
    ...
    for (int e = 0; e < NUM_EPOCHS; e++)
    {
            ...
            for (int c = 0; c < NUM_CLASSES; c++)
            {
               ...
               // printf("%f\n", learningRate);
                ...
                if (c == (labels[j] - 1))
                {
                    for (int d = 0; d < (FEATURE_COLS + 1); d++)
                    {
                        float temp = (features[j][d]*(1.0f - p_c_x_c) - (2 * REG_PARAM * beta[c][d]));
                        beta[c][d] += learningRate*(features[j][d]*(1.0f - p_c_x_c) - (2 * REG_PARAM * beta[c][d]));
                    }
                } else {
                    for (int d = 0; d < (FEATURE_COLS + 1); d++)
                    {
                        float temp = (features[j][d]*(0.0f - p_c_x_c) - (2 * REG_PARAM * beta[c][d]));
        ...
        for (int j = 0; j < NUM_HELDOUT_FILES * ROWS_PER_FILE; j++)
        {
            ...
            heldLogLikelihood += dotProductV[heldLabels[j] - 1] - (maxDotProduct + logf(z));
        }
