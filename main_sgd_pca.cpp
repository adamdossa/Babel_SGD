...
//
//  Runs Stochastic Gradient Descent, but first multiplies input data by PCA loading matrix read from file
...
//Number of features to reduce to via PCA loadings
#define RANK 60
//#define NUM_FILES 1
...
int main(int argc, const char * argv[])
{
    ...
    for (int i = 0; i < 100; i++)
    {
        snprintf(logFileName, sizeof(char) * 200,"%s/log/log_pca-%d-%d-%d-%f-%f.txt.%d", currentDirectory, rank, NUM_EPOCHS, NUM_FILES, regParam, initialLR, i);
    ...
    char loadName[200];
    snprintf(loadName, sizeof(char) * 200,"%s/loadings-%d.load", currentDirectory, NUM_FILES);
    ...
    int startingEpoch = 0;
    char lsName[200];
    snprintf(lsName, sizeof(char) * 200,"ls %s/beta/beta_pca-%d-%s-%d-%f-%f.out | grep 'beta' | cut -d'-' -f3", currentDirectory, rank, "*", NUM_FILES, regParam, initialLR);
    ...
    if (startingEpoch > 0)
    {
        ...
        snprintf(lsName, sizeof(char) * 200,"%s/beta/beta_pca-%d-%d-%d-%f-%f.out", currentDirectory, rank, startingEpoch - 1, NUM_FILES, regParam, initialLR);
    ...
    for (int e = startingEpoch; e < NUM_EPOCHS; e++)
    {
        ...
        char fName[200];
        snprintf(fName, sizeof(char) * 200,"%s/beta/beta_pca-%d-%d-%d-%f-%f.out", currentDirectory, rank, e, NUM_FILES, regParam, initialLR);
