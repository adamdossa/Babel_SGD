...
//  Can change to read in different beta files as needed
...
int main(int argc, const char * argv[])
{
    ...
    for (int i = 0; i < 100; i++)
    {
        snprintf(logFileName, sizeof(char) * 200,"%s/log/log_likelihood-%d-%d-%d-%f-%f.txt.%d", currentDirectory, rank, START_EPOCH, NUM_FILES, regParam, initialLR, i);
    ...
    for (int e = startingEpoch; e < 99; e++)
    {
        ...
        snprintf(lsName, sizeof(char) * 200,"%s/beta/beta_sgd-%d-%d-%d-%f-%f.out", currentDirectory, rank, e, NUM_FILES, regParam, initialLR);
