...
#define INITIAL_LR_LARGE 0.001f //e.g. used to goto 42 mapping (e.g. main_sgd_42)
...
#define REG_PARAM_LARGE 1000.0f
...
int main(int argc, const char * argv[])
{
    ...
    const char * labelSmallDir = "labels_42";
    ...
    for (int i = 0; i < 100; i++)
    {
        snprintf(logFileName, sizeof(char) * 200,"%s/log_sub_likelihood-%d-%d-%d-%f-%f-%f-%f.txt.%d", currentDirectory, rank, START_EPOCH, NUM_FILES, regParamLarge, initialLRLarge, regParamSmall, initialLRSmall, i);
    ...
    char fName[200];
    int n;
    snprintf(fName, sizeof(char) * 200,"%s/%s/label_mapping.lab", rootDirectory, labelSmallDir);
    ...
    for (int e = startingEpoch; e < 99; e++)
    {
        ...
        if (doTrainLikelihood) {
            ...
            for (int j = 0; j < noOfTrainingExamples; j++)
            {
                ...
                float secondProb = dotProductSubV[subClass] - (maxDotProduct + logf(z));
