...
int main(int argc, const char * argv[])
{
    ...
    for (int i = 0; i < 100; i++)
    {
        snprintf(logFileName, sizeof(char) * 200,"%s/log_bound-%d-%d-%d-%f-%f.txt.%d", currentDirectory, rank, NUM_EPOCHS, NUM_FILES, regParam, initialLR, i);
    ...
    int startingEpoch = 0;
    char lsName[200];
    snprintf(lsName, sizeof(char) * 200,"ls %s/theta_bound-%d-%s-%d-%f-%f.out | grep 'theta_bound' | cut -d'-' -f3", currentDirectory, rank, "*", NUM_FILES, regParam, initialLR);
    ...
    if (startingEpoch > 0)
    {
        ...
        snprintf(lsName, sizeof(char) * 200,"%s/theta_bound-%d-%d-%d-%f-%f.out", currentDirectory, rank, startingEpoch - 1, NUM_FILES, regParam, initialLR);
...
//                    float rat2 = 1.0f - rat1;
        ...
        char fName[200];
        snprintf(fName, sizeof(char) * 200,"%s/theta_bound-%d-%d-%d-%f-%f.out", currentDirectory, rank, e, NUM_FILES, regParam, initialLR);
