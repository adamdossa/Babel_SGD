...
//
//  Computes covariance matrix for input data (for PCA analysis)
...
int main(int argc, const char * argv[])
{
    ...
    for (int i = 0; i < 100; i++)
    {
        snprintf(logFileName, sizeof(char) * 200,"%s/log_cov-%d.txt.%d", currentDirectory, NUM_FILES, i);
    ...
    for (int i = 0; i < NUM_FILES; i++) {
        ...
        for (int j = 0; j < n; j++)
        {
            ...
            fread(&m,4,1,pFile);
            m = ntohl(m);
            features[readSoFar + j] = (float *) malloc(sizeof(float) * m);
    ...
    {    
        ...
        {
            float covI = 0.0f;
            for (int k = 0; k < noOfTrainingExamples; k++)
            {
                covI += features[k][i] * features[k][j];
                fprintf(outputFile, "Summing: %d, %d: %f\n", i, j, features[k][i] * features[k][j]);
            }
            fflush(outputFile);
            if (debugCov)
            {
                fprintf(outputFile, "COVI: %d, %d: %f\n", i, j, covI);
                fflush(outputFile);
            }
            covMat[i][j] = covI;
            covMat[j][i] = covI;
    ...
    char fName[200];
    snprintf(fName, sizeof(char) * 200,"%s/cov-%d.cov", currentDirectory, NUM_FILES);
