...
int main(int argc, const char * argv[])
{
    ...
    for (int i = 0; i < 100; i++)
    {
        snprintf(logFileName, sizeof(char) * 200,"%s/log/log_error-%d-%d-%d-%f-%f.txt.%d", currentDirectory, rank, epoch, NUM_HELDOUT_FILES, regParam, initialLR, i);
    ...
    for (int i = 0; i < 100; i++)
    {
        snprintf(logFileName, sizeof(char) * 200,"%s/log/cmatrix-%d-%d-%d-%f-%f.txt.%d", currentDirectory, rank, epoch, NUM_HELDOUT_FILES, regParam, initialLR, i);
