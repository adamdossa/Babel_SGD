...
int main(int argc, const char * argv[])
{
    ...
    for (int i = 0; i < 100; i++)
    {
        snprintf(logFileName, sizeof(char) * 200,"%s/log/log_sgd_42-%d-%d-%d-%f-%f.txt.%d", currentDirectory, rank, NUM_EPOCHS, NUM_FILES, regParam, initialLR, i);
    ...
    for (int i = 0; i < NUM_FILES; i++) {
        ...
        for (int j = 0; j < n; j++)
        {
            labels[readSoFar + j] = labelMapping[ntohl(labels[readSoFar + j])];
        }
        readSoFar += n;
        fclose(pFile);
    }
    ...
    if (doTestLikelihood) {
        ...
        for (int i = 0; i<NUM_HELDOUT_FILES; i++) {
            ...
            for (int j = 0; j < n; j++)
            {
                heldLabels[readSoFar + j] = labelMapping[ntohl(heldLabels[readSoFar + j])];
            }
            readSoFar += n;
            fclose(pFile);
        }
    ...
    int startingEpoch = 0;
    char lsName[200];
    snprintf(lsName, sizeof(char) * 200,"ls %s/beta/beta_sgd_42-%d-%s-%d-%f-%f.out | grep 'beta' | cut -d'-' -f3", currentDirectory, rank, "*", NUM_FILES, regParam, initialLR);
    ...
    if (startingEpoch > 0)
    {
        ...
        snprintf(lsName, sizeof(char) * 200,"%s/beta/beta_sgd_42-%d-%d-%d-%f-%f.out", currentDirectory, rank, startingEpoch - 1, NUM_FILES, regParam, initialLR);
    ...
    for (int e = startingEpoch; e < NUM_EPOCHS; e++)
    {
        ...
        char fName[200];
        snprintf(fName, sizeof(char) * 200,"%s/beta/beta_sgd_42-%d-%d-%d-%f-%f.out", currentDirectory, rank, e, NUM_FILES, regParam, initialLR);
